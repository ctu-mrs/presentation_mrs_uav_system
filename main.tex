...
\frametitle{The MRS UAV System}
  ...
  \begin{block}{MRS UAV System block diagram}
    \begin{figure}
      \includegraphics[width=0.6\textwidth]{./fig/mrs_system_new.png}
    \end{figure}
  \end{block}
...
\begin{frame}
  \frametitle{ROS2 -- Robot Operating System}

  \begin{itemize}
    \item middleware allowing communication between independent programs even over the network
    ...
    \item almost universally supported by sensor manufacturers
    \item used globally by the research community
    \item integration through the Linux shell
    ...
    \item integrates to robotic simulators: Gazebo, Coppelia (V-REP), AirSim, FlightForge
  \end{itemize}
...
\begin{frame}
  ...
  \only<3>{
    \begin{block}{High-fidelity visuals --- requires capable rendering engine}
      \begin{center}
        \mymovie[autostart,loop]{
          \includegraphics[width=0.70\textwidth]{./fig/berlin_flightforge_thumbnail.jpg}
        }{./videos/berlin_flightforge.mp4}\\
        Video: \url{https://youtu.be/_XCSalkl1Ys}
      \end{center}
    \end{block}
  }
...
\begin{frame}
  ...
  \begin{block}{Native installation using ROS Jazzy}
  ...
  \begin{block}{Documentation}
    \begin{center}
      \large {\color{blue} https://ctu-mrs.github.io}
    \end{center}
  \end{block}
...
\begin{block}{Starting to get usable}
  \begin{itemize}
    \item design regressions at many places
    \item performance worse than ROS1 but ok
    \item more complex for a \emph{generic user} to work with
  \end{itemize}
\end{block}

\begin{block}{State of the transition}
  \begin{center}
...
\begin{block}{Available in ROS2}
  \begin{itemize}
    ...
    \item MRS Gazebo stack
    \item GNSS-denied flight (LiDAR, VIO)
    \item Octomap Mapping \& Planning
    \item Pointcloud utils
...
\begin{block}{To be done}
  \begin{itemize}
    ...
    \item {\color{red} Some of the QoL utilities}
  \end{itemize}
\end{block}
